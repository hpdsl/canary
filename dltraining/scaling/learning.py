...
code_deployed = time.time()-initTime
print("MAKESPAN ::> Code deployed -> Time = {0}.s | Memory = {1}.MB".format(code_deployed, resource.getrusage(resource.RUSAGE_SELF).ru_maxrss / 1024))

class PrintLR(callbacks.Callback):
    def on_epoch_end(self, epoch, logs=None):
        print("MAKESPAN CANARY::> State {0} -> Time = {1}.s | Memory = {2}.MB".format(epoch, time.time()-initTime, resource.getrusage(resource.RUSAGE_SELF).ru_maxrss / 1024))
...
def main(args):
    ...
    data_loaded = time.time()-initTime
    print("MAKESPAN CANARY::> Data Loaded -> Time = {0}.s | Memory = {1}.MB".format(loaded_time, resource.getrusage(resource.RUSAGE_SELF).ru_maxrss / 1024))
    ...
    data_processed = time.time()-initTime
    print("MAKESPAN CANARY::> Data Processed -> Time = {0}.s | Memory = {1}.MB".format(data_processed, resource.getrusage(resource.RUSAGE_SELF).ru_maxrss / 1024))
    ...
    ready_training = time.time()-initTime
    print("MAKESPAN CANARY::> Ready for Training -> Time = {0}.s | Memory = {1}.MB".format(ready_training, resource.getrusage(resource.RUSAGE_SELF).ru_maxrss / 1024))
    history = model.fit(
        train_features_flat,
        train_label,
        epochs=NB_EPOCHS,
    ...
    function_complete = time.time()-initTime
    print("MAKESPAN CANARY::> Function completion -> Time = {0}.s | Memory = {1}.MB".format(function_complete, final_memory))
    output = {'CANARY-RESULT': ' -> Deployed={0}.s | loaded={1}.s | processed={2}.s | ready={3}.s | complete={4}.s'.format(code_deployed, data_loaded, data_processed, ready_training, function_complete)}
    return output
